...
**Axiom codebase audit — preservation-first**

This document is a working audit of the Axiom codebase written with a single guiding rule: assume everything is valuable until proven otherwise. The goal is not to propose a rewrite, invent a new vocabulary, or simplify the product into something smaller. The goal is to surface what already works, route it to the right part of the product surface, and call out only what is genuinely dead or duplicative.
...
These are working, tested subsystems that must not be touched unless there is a specific bug to fix. They form the backbone of Axiom's product value.

### 1.1 Core RAG pipeline

`axiom_app/engine/querying.py` · `axiom_app/engine/streaming.py` · `axiom_app/services/vector_store.py`
...
`axiom_app/services/response_pipeline.py` · `axiom_app/engine/streaming.py`
...
`axiom_app/engine/streaming.py` → `_generate_sub_queries()`
...
`axiom_app/services/vector_store.py` · `axiom_app/services/index_service.py`
...
`axiom_app/services/vector_store.py`
...
`axiom_app/utils/knowledge_graph.py`
...
`axiom_app/services/session_repository.py`
...
`axiom_app/services/trace_store.py` · `axiom_app/services/stream_replay.py`
...
`axiom_app/services/skill_repository.py` · `axiom_app/services/runtime_resolution.py` · `skills/`
...
`axiom_app/models/brain_graph.py` · `apps/axiom-web/app/brain/` · `axiom_app/api/app.py` → `/v1/brain/graph`
...
`axiom_app/utils/llm_providers.py` · `axiom_app/utils/embedding_providers.py` · `axiom_app/utils/llm_backends.py`
...
`axiom_app/utils/llm_backends.py` · `axiom_app/services/local_model_registry.py` · `axiom_app/services/local_llm_recommender.py` · `apps/axiom-web/app/gguf/`
...
`axiom_app/api/sessions.py` → `POST /v1/sessions/{id}/feedback` · `axiom_app/api/app.py` → `POST /v1/runs/{run_id}/actions`
...
`axiom_app/api/app.py`

`AXIOM_API_TOKEN` enables Bearer token authentication on all protected endpoints. `AXIOM_API_CORS_ORIGINS` controls allowed origins. `api_key_*` fields are rejected by default at write time (require `AXIOM_ALLOW_API_KEY_WRITE=1`). Error mapping (`ValueError` → 400, `RuntimeError` → 503) is consistent across all routes.
...
`apps/axiom-web/app/gguf/` · `axiom_app/utils/llm_backends.py`
...
`axiom_app/services/profile_repository.py` · `axiom_app/models/parity_types.py` → `AgentProfile`
...
`axiom_app/engine/querying.py` → `query_direct()` · `POST /v1/query/direct`
...
`apps/axiom-web/app/diagnostics/` · `GET /v1/traces/{run_id}`
...
### 4.2 `apps/axiom-reflex/` — superseded proof-of-concept
...
`apps/axiom-web/app/design/page.tsx` is a design-system component playground. It is useful for contributors but should not be reachable via the main navigation in production builds.
...
### 4.5 `apps/axiom-web-lite/` (Astro) — unclear product role
...
**Action:** Evaluate whether `axiom-web-lite` serves a specific purpose (e.g., an embedded docs site or a lightweight demo). If there is no active use case, archive or remove. If it is retained, add a `README.md` that explains its scope relative to the main `axiom-web`.

### 4.6 `axiom_app/services/heretic_service.py` — legacy controller dependency, no API route

`heretic_service.py` wraps the `heretic-llm` CLI (a HuggingFace model abliteration tool) via subprocess calls to avoid importing its AGPL-licensed code. It is imported in `axiom_app/controllers/app_controller.py`, which is the legacy Qt MVC controller layer. No FastAPI route calls it, no web UI page surfaces it, and no skill or settings key references it. The controller itself is only exercised by legacy tests and is being deprecated as part of the one-interface transition (ADR 0004).

**Action:** Move to a `scripts/` or `tools/` directory to signal it is a developer utility, not a product feature. Alternatively, gate it behind an explicit `AXIOM_ENABLE_HERETIC=1` environment flag. Either way, address it as part of the broader `app_controller.py` legacy cleanup rather than in isolation.
...
### 4.8 `apps/axiom-reflex/axiom_reflex.py` reference to `python main.py`

Even if the Reflex app is kept as an archive, `axiom_reflex.py` contains a comment referencing `python main.py` as the Qt desktop entry point. This contradicts ADR 0004. Any archived copy should have the comment corrected or removed so it does not re-surface as documentation.
