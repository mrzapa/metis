...
>
> **2026-05-01 reap note:** the `brain-graph-3d.tsx` surface listed in step 4 was never mounted on a route in v1 — the 3D brain-graph code path was an unreachable subtree, reaped under M01 §4.9 along with `react-force-graph-3d`. The H₁ rings + scaffold-edge-glow code never reached users. The backend scaffold (steps 1–3) and the 2D constellation scaffold (step 6) are both live; only the 3D visual surface in step 4 was dormant. If a future iteration wants the H₁ rings visible, design lives in git history (`brain-graph-3d.tsx` last commit before delete) — re-introduction needs a route under `app/brain/page.tsx`.
