...
| M16 | **Personal evals** — track companion improvement on user's specific tasks | 🌱 | In progress | [`plans/personal-evals/plan.md`](personal-evals/plan.md) | claude/m16-phase2-eval-store (Phase 1 ADRs 0016/0017/0018 landed via PR #594, `a9e57c6`, 2026-05-01) | 2026-05-01 | M13 |
...
| M21 | **UI critical-eye triage** — bug bash from full-app QA walk: hydration mismatch, dead routes, request-storm, nav inconsistency, overlay/safe-area issues | 🔧 | In progress | [`plans/ui-critical-triage/plan.md`](ui-critical-triage/plan.md) | P0 batch (#1, #2, #4, #5) landed via PR #588 (`7604156` + Codex `3f732cf`). P1 #6 request-storm dedup landed via PR #597. Phase 5 constellation aesthetic pivot (#23–#27) landed via PR #600 (`500bc8e`, 2026-05-02) — bundles five user-critique fixes + design reversal of two M02 / ADR 0006 implementation choices ([ADR 0006 *Addendum — 2026-05-01 (M21 Phase 5)*](../docs/adr/0006-constellation-design-2d-primary.md)). Remaining: P1 #7–#9, P2 #10–#18, P3 #19–#22 (all unclaimed). All prior claims cleared | 2026-05-02 | — |
...
| M23 | **Companion controls** — tone presets + memory inspector in `/settings → Companion`; closes Shape of AI gaps #1 + #2 | 🌱 | Landed | [`plans/companion-controls/plan.md`](companion-controls/plan.md) (design: [`docs/plans/2026-05-03-companion-controls-design.md`](../docs/plans/2026-05-03-companion-controls-design.md), impl: [`docs/plans/2026-05-03-companion-controls-implementation.md`](../docs/plans/2026-05-03-companion-controls-implementation.md)) | Landed via [PR #610](https://github.com/mrzapa/metis/pull/610) (`1f3f8af`, 2026-05-03) — 30 commits, 6 phases + 4 review-response fixes. Backend pytest 1601/12, frontend vitest 740/10, tsc clean | 2026-05-03 | M13 |
| M24 | **Constellation IA reset (UI)** — kill faculty anchors, content-cluster placement, AI-suggested Add flow, central-star Everything chat | 🌌 | Ready for review | [`plans/constellation-ia-reset/plan.md`](constellation-ia-reset/plan.md) (design: [`docs/plans/2026-05-03-constellation-ia-reset-design.md`](../docs/plans/2026-05-03-constellation-ia-reset-design.md), ADR: [`docs/adr/0019-constellation-ia-content-first-projects.md`](../docs/adr/0019-constellation-ia-content-first-projects.md)) | All 6 phases shipped on `claude/m24-impl` (~35 commits). Vitest 761/10, backend pytest 1621/12, tsc clean. Browser-preview QA deferred to main repo (Turbopack incompatibility with worktree junctions); 7-step walk in [Task 6.5](../docs/plans/2026-05-03-constellation-ia-reset-m24-implementation.md) covers cluster layout, AddStarDialog, Everything chat, observatory cleanup, layout persistence, reduced-motion, degraded backend | 2026-05-03 | — |
| M25 | **Projects + Forge integration** — drawable scoped workspaces (click-to-select-then-confirm), per-Project Forge config, Project chat | 🌌🧠 | Draft | [`plans/projects-and-forge-integration/plan.md`](projects-and-forge-integration/plan.md) (same design + ADR as M24) | — | 2026-05-03 | M24 |
| M26 | **Faculty backend taxonomy removal** — replace faculty-gap-scoring with cluster-gap-scoring; eval-gated flip | 🔧🧠 | Draft | [`plans/faculty-backend-removal/plan.md`](faculty-backend-removal/plan.md) (same design + ADR as M24) | — | 2026-05-03 | M24, M25, M16 |
...
- **M02** — 2D constellation refactor. Touches: `apps/metis-web/components/home/*`,
  ...
  `star-name-generator.ts`. **Heavy overlap with M12.** **Faculty-anchor placement decision partially superseded by ADR 0019 (M24+); archetype + tiered-naming policy preserved.**
...
- **M23** — Frontend-mostly Companion-pillar feature. Extends `AssistantIdentity` with one `tone_preset` field, adds `TONE_PRESETS` dict + seed-resolution rule, three new `AssistantRepository` delete methods + matching Litestar `DELETE` routes. Touches `apps/metis-web/app/settings/page.tsx` (Companion tab restructure), three new components under `apps/metis-web/components/settings/companion/*` (`personality-card.tsx`, `memory-inspector.tsx`, `memory-stats-row.tsx`), and `metis-companion-dock.tsx` (one `Settings ↗` deep-link in the minimised header). Closes Shape of AI audit gaps #1 (Voice and tone + Personality) and #2 (Memory inspector); flip both ❌ → ✅ at Phase 6. **No overlap** with M21 (bug-bash) or M16 (eval store). Builds on M13 (Landed) memory primitives.
- **M24** — Cosmos-pillar UI pivot. Replaces `FACULTY_CONCEPTS` / faculty-ring placement with content-embedding-cluster placement; new `AddStarDialog` (file picker + AI-suggested top-5 + Create-new); new `EverythingChatSheet` from central METIS star click. New backend: `star_clustering_service.py` + `GET /v1/stars/clusters` + `POST /v1/stars/recommend`. Touches `apps/metis-web/app/page.tsx` (heavy), `components/home/*` (new dialogs), `components/constellation/star-observatory-dialog.tsx` (remove `faculty-glyph-panel`), `lib/landing-stars/star-name-generator.ts` (retire 8 named landmarks in Phase 6). Backend faculty signal stays as invisible internal — `comet_decision_engine`, `autonomous_research_service`, `star_nourishment_gen`, Tribev2 untouched. M02 / ADR 0006 partially superseded; ADR 0019 written before start.
- **M25** — Cosmos + Cortex. New `Project` data model (`metis_app/models/project_types.py` + `metis_app/services/project_repository.py`) + 7 HTTP routes (CRUD + project-scoped chat). Frontend: click-to-select-then-confirm UX (selection-mode state + pulse + floating Confirm), cubic-Bezier line rendering between member stars, force-directed pull layered on M24 cluster placement, Project detail panel with member management, Project-scoped chat sheet, per-Project Forge config. Wires M24's recommender to boost same-Project stars. Depends on M24.
- **M26** — Cross-cutting backend rewrite. Replaces faculty-gap-scoring with cluster-gap-scoring across `metis_app/services/{autonomous_research_service,comet_decision_engine,news_ingest_service,brain_pass,star_nourishment_gen}.py`, `metis_app/models/{star_nourishment,comet_event}.py`, `metis_app/services/news_feed_repository.py` (schema migration), and Tribev2 classifier. M04 reverse-curriculum logic preserved; signal source replaced. **Eval-gated:** must achieve research-quality parity with faculty-gap signal on M16 evals before final flip. Variable Phase 3 length depending on eval iteration. Depends on M24, M25, M16.
