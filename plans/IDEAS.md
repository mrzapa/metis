...
### Constellation IA reset — kill faculty anchors, files-first Add flow, drawable Projects, central-star global chat

- **Source:** User's own product critique, 2026-05-03. References [ChatGPT Projects](https://help.openai.com/en/articles/10169521-using-projects-in-chatgpt) and [Claude Projects](https://support.claude.com/en/articles/9517075-what-are-projects) as the design vocabulary for the "drawable lines = scoped chat workspace" concept.
- **Ask:** Four bundled changes —
  - **(a)** Remove the **faculty/sigil system** as the constellation's organising principle (M21 Phase 5 already removed surface labels + lens-flare on the canvas; the user wants the underlying faculty-as-anchor concept retired).
  - **(b)** Rework the **Add button** flow: clicking `+` opens a file-picker / paste-text dialog → AI suggests the **best existing star** to attach the content to (based on architecture/index type/content), or offers to create a new star. Replaces today's flow where the user manually assigns archetype + faculty in the Star Observatory.
  - **(c)** Introduce **drawable constellation lines as Projects**: users draw lines between stars to form scoped workspaces — file-scoped chat with per-project instructions and memory, à la ChatGPT/Claude Projects.
  - **(d)** Make the **central METIS star** the entry point for "chat with all my files" (global cross-project chat — the everything-chat counterpoint to Project-scoped chat).
  - Plus: clarify how the existing **Forge + Chat** surfaces fit alongside the new shape.
- **Context:** Direct user message. The user calls out that M21 Phase 5's faculty removal "wasn't done properly" — they want the faculty/sigil system retired as a Cosmos organising principle, not just visually quietened. References ChatGPT/Claude Projects as the model: a Project = a persistent scoped workspace (files + chats + per-project instructions) where the AI sees only the project's context. Free Claude tier gets 5 projects, paid is unlimited; ChatGPT Projects is Plus/Team/Enterprise. Both let the user upload files into a project and have all chats inside that project see those files as context. Both let you save responses back into the project as sources.
- **Filed:** 2026-05-03 by claude/constellation-ia-intake
- **Triage:**
  - **What it is:** A *foundational IA pivot* for the Cosmos pillar — bigger than any single visual change. The current constellation places stars by faculty (Perseus, Auriga, Draco, Hercules, Gemini, Big Dipper, Lyra, Boötes — the 8 canonical landmarks per [ADR 0006](../docs/adr/0006-constellation-design-2d-primary.md)). Faculties also tag almost every backend pipeline: 13 backend modules reference faculty IDs (`metis_app/services/news_ingest_service.py`, `comet_decision_engine.py`, `autonomous_research_service.py`, `brain_pass.py`, `star_nourishment_gen.py`, `news_feed_repository.py`, the `WorkspaceOrchestrator`, the assistant routes, plus model files). The frontend `app/page.tsx` has 129 hits for `faculty|Faculty`, anchored on `FACULTY_CONCEPTS` at line 176 — the type that drives node placement, hit-zones, and rendering across the whole canvas. Removing faculties as the *organising* metaphor on the canvas is one task; removing them as a *backend taxonomy* is another (multi-week refactor through news ingestion, comet decisions, autonomous research, and star nourishment).
    
    The user's request is best read as: replace the faculty-as-organising-metaphor in the Cosmos UI with a *content-first* IA, and add a new line-drawing primitive that creates persistent scoped chat workspaces. ChatGPT Projects and Claude Projects are the cited references — they're scoped containers (files + chats + per-project instructions) where the AI sees only the project's context. The Metis equivalent is "select N stars by drawing a line between them, those stars' content scopes the chat, the line itself becomes a Project entity with its own chat history and per-project memory." The central METIS star becoming "chat with all files" is the natural counterpoint: Projects scope chat to a subset; the centre is the everything-chat.
  - **Pillar fit:** Heavy **Cosmos** (🌌) — the constellation's IA changes fundamentally. Substantial **Cortex** (🧠) — Projects are a chat-scoping primitive, requires backend work for file-scoped retrieval and per-project memory. Touches **Companion** (🌱) — the central-star "all files" chat is effectively a different mode of companion engagement. Cross-cutting infrastructure work for the faculty-removal cleanup.
  - **Overlap:** Massive.
    - **M02 (Landed; ADR 0006)** — the faculty constellation system *is* M02. Removing faculty anchors reverses M02's primary IA decision. ADR 0006 explicitly preserves "the constellation home (8 faculty constellations) and their classical naming". This proposal reverses that.
    - **M21 (In progress; Phase 5 landed)** — already partial. Phase 5 removed faculty title text + central-star lens flare + classical-name hover. The user is saying that wasn't enough — go further on the underlying anchor concept, not just the surface paint.
    - **M12 (Landed)** — interactive star catalogue. The new Add flow + Star Observatory rework would touch the catalogue's inspector / search surface.
    - **M13 (Landed)** — Seedling + comet ingestion: news comets get tagged with faculty IDs in `comet_decision_engine.py`. Removing faculties from ingestion is a cascade fix.
    - **M14 (Landed)** — Forge's technique cards are a different kind of scoped surface. The user explicitly asks how Forge + Chat link to the new Projects concept. Likely answer: Projects can attach techniques (e.g. "this Project uses IterRAG by default", "this Project has Heretic enabled"), so the Forge becomes a per-Project palette.
    - **M15 (Draft)** — Pro tier: ChatGPT/Claude Projects are explicit Pro/Plus features. If Metis Projects are the differentiator, they're load-bearing for the Pro pitch.
    - **M16 (In progress)** — Personal evals: per-Project evals would naturally extend M16's eval store to a per-project scope.
    - **M17 (Landed)** — Network audit: per-Project network egress reporting is a natural extension.
    - **M22 (Landed)** — Comet headline labels: comets are faculty-tagged today; if faculties go away, comets need a new placement metaphor.
    - **VISION** — the Cosmos pillar paragraph says "Stars are placed by faculty (Perception, Knowledge, Memory, Reasoning, Skills, Strategy, Personality, Values)." That sentence becomes false. VISION.md needs an amendment.
  - **Risks worth noting:**
    - **ADR 0006 reversal.** Needs a new ADR (or substantial addendum to 0006) explaining what replaces faculty-as-anchor. Without that, future agents will rebuild what's removed.
    - **Backend faculty references are deeper than the surface suggests.** 13 backend modules touch faculty IDs. Removing faculties as a UI organising principle doesn't necessarily require removing them from the backend (they could become a hidden-from-UI internal classifier used by autonomous-research curriculum, news-comet decision engine, etc.). Cheapest path: kill the UI surface, keep the backend taxonomy as internal-only. Cleanest path: purge end-to-end. User's ask is ambiguous on which. Default triage assumption: kill UI surface, keep backend taxonomy hidden — but flag for the brainstorm.
    - **Star placement without faculties.** Today a star's `(x, y)` is computed from its faculty anchor + jitter. If faculties go away, what determines placement? Options: cluster by content embedding (k-means / UMAP layout), force-directed by document-similarity, fixed grid, user-drag-to-place, hybrid (auto-suggest + user-adjust). Each has distinct UX trade-offs. Multi-day design choice.
    - **Projects vs. existing index/session model.** Today a chat session runs against an *index* (a built RAG store). Projects would scope chat to a *set of stars* (which themselves attach to indexes). Is a Project = a virtual index? A list of index IDs the chat can pull from? A first-class entity with its own indexed content? Three plausible answers; needs design.
    - **Drawable lines on canvas.** New input modality (click-and-drag-to-draw) on a canvas that today is click-to-select / drag-to-pan. Mode switching, hit detection, undo, persistence, deletion — substantial UI work.
    - **Migration of existing user data.** Existing user stars are placed by faculty. After this change, they need new positions. M02 Phase 8.5 solved archetype migration via deriving on render — does similar derive-on-render logic work for placement, or do existing stars need a one-shot migration to faculty-free coordinates?
    - **VISION pillar tension.** "Stars are placed by faculty" is a load-bearing sentence in the Cosmos pillar. Cleaning this up requires the user's go-ahead on a VISION amendment, not just a code change.
    - **No backwards-compat free lunch.** Existing landing-page muscle memory (8 faculty quadrants, faculty-shaded zones, "the cluster of stars over there is Perception") will be wrong overnight. The user has accepted disruption explicitly; recording it for completeness.
  - **Recommendation:** **Brainstorm before promote.** This is too big to file as a single milestone without design exploration. Specifically:
    1. Run a focused brainstorming pass (`superpowers:brainstorming`) on each sub-change individually:
       - **(a) Faculty removal scope** — UI-only purge or end-to-end taxonomy removal? What replaces faculty as a placement principle? Does the backend faculty taxonomy stay internal?
       - **(b) Add flow** — what data signals drive "best star to add to" recommendations? Embedding similarity to existing star content? Existing-index match? Content-type detection? Sub-star creation when no good match exists?
       - **(c) Projects** — first-class entity with own DB row or a saved-search concept over star IDs? Persistence model? Draw-line UX (click-each-star-then-confirm? drag-rope-around-stars? rectangle-select?)? Deletion + undo?
       - **(d) Central-star chat** — does this differ semantically from the existing `/chat` Direct/RAG modes, or is it a new index-spanning mode (RAG over the union of all attached indexes)?
       - **Forge + Chat integration** — Project-scoped technique enablement? Project-scoped chat history? Project-level system instructions like Claude Projects?
    2. Produce a new **ADR** (proposal: "ADR 0019 — Constellation IA: content-first, drawable Projects") or a substantial addendum to ADR 0006 reversing the faculty-anchor decision and naming what replaces it.
    3. Draft a **VISION.md amendment** — at minimum the Cosmos pillar paragraph and the "How an AI grows in METIS" section. The promise that stars are placed by faculty becomes "stars are placed by content"; the Project primitive becomes part of "feed it" / "let it dream"; etc.
    4. **Most likely outcome:** **Promote** as a pair of milestones, probably **M24 — Constellation IA reset** (sub-changes a + b + d) and **M25 — Projects as drawable scopes** (sub-change c + Forge integration). Each ~2–4 weeks. Together plausibly ~a quarter of work end-to-end if both are taken to ship.
    5. **Less likely:** **Park** until M15 (Pro tier launch) is closer — Projects are an explicit Pro feature on competitor products; bundling the rollout with launch could be coherent. But the constellation IA fix is signalled as needed *now*, so this would create friction.
    6. **Reject** is wrong here despite the ADR reversal — the user is saying "M02's decisions didn't pan out; here's the next iteration", and that's what the codebase should support.
  - **Rough scope:**
    - Brainstorm + ADR + design docs: **~3–5 days**.
    - **Lighter slice (M24 only — UI-only faculty purge + new Add flow + central-star chat):** **~2 weeks**.
    - **Full slice (M24 + M25 — adds Projects + Forge integration + backend faculty taxonomy cleanup):** **~6–10 weeks**.
    - VISION.md amendments + ADR: **~half-day each**.
- **Decision:** **Brainstormed + Promoted as M24/M25/M26 (2026-05-03).** Five-question brainstorm pass converged on: faculty UI purge (Q1 → trajectory B / staged); content-embedding-cluster placement with Project pull (Q2 → option D); AI-suggested top-5 stars + Create-new side-by-side (Q3 → D+1); Project = saved selection over star IDs with click-to-select-then-confirm UX (Q4 → A + click-to-select per user clarification); central-star = Everything chat, dock keeps companion-chat, per-Project Forge config (Q5 → 1 + B). Trajectory promoted as three milestones — **M24 — Constellation IA reset (UI)** ([`plans/constellation-ia-reset/plan.md`](constellation-ia-reset/plan.md), ~2 weeks), **M25 — Projects + Forge integration** ([`plans/projects-and-forge-integration/plan.md`](projects-and-forge-integration/plan.md), ~2.5 weeks, depends on M24), **M26 — Faculty backend taxonomy removal** ([`plans/faculty-backend-removal/plan.md`](faculty-backend-removal/plan.md), ~3.5 weeks, eval-gated, depends on M24+M25+M16). Combined design at [`docs/plans/2026-05-03-constellation-ia-reset-design.md`](../docs/plans/2026-05-03-constellation-ia-reset-design.md); M24 task-by-task plan at [`docs/plans/2026-05-03-constellation-ia-reset-m24-implementation.md`](../docs/plans/2026-05-03-constellation-ia-reset-m24-implementation.md). New ADR: [`docs/adr/0019-constellation-ia-content-first-projects.md`](../docs/adr/0019-constellation-ia-content-first-projects.md) — partially supersedes ADR 0006's faculty-anchor placement decision; archetype + tiered-naming policy preserved. VISION.md amended: Cosmos pillar paragraph rewritten ("stars are placed by content"); "How an AI grows" gets the Projects loop. Intake PR: [#612](https://github.com/mrzapa/metis/pull/612).

### Shape of AI — pattern-coverage audit of the Metis frontend

- **Source:** https://www.shapeof.ai/ (Emily Campbell, 2025; CC BY-NC-SA). Curated catalogue of ~50 AI-native UX patterns organised into six families: **Wayfinders** (Example gallery, Follow up, Initial CTA, Nudges, Prompt details, Randomize, Suggestions, Templates), **Inputs / Prompt actions** (~15 incl. Inline Action, Regenerate, Restructure, Summary, Synthesis, Transform), **Tuners** (Attachments, Connectors, Filters, Model management, Modes, Parameters, Preset styles, Prompt enhancer, Saved styles, Voice and tone), **Governors** (Action plan, Branches, Citations, Controls, Cost estimates, Draft mode, Memory, References, Sample response, Shared vision, Stream of Thought, Variations, Verification), **Trust builders** (Caveat, Consent, Data ownership, Disclosure, Footprints, Incognito Mode, Watermark), **Identifiers** (Avatar, Color, Iconography, Name, Personality). Each pattern page ships a description, design considerations, related-patterns map, and worked examples drawn from real shipped products (Adobe, Granola, Perplexity, ChatGPT, Notion, Lovable, V0, Dovetail, Sana, Intercom Fin, etc.).
- **Ask:** Analyse Shape of AI in detail and review it against the Metis frontend; recommend improvements.
- **Context:** User pointed at the site as "a detailed guide on what AI apps should look like" and wants the catalogue used as a yardstick for the Metis UI. Adjacent prior work is heavy: M01 *Web UI new-user walkthrough* (2026-04-25) + M21 *UI critical-eye triage* (Phases 1, 2 partial, 5 landed) have already surfaced ~30 bugs that — read in this lens — are nearly all violations of specific Shape-of-AI patterns (broken Wayfinders, missing Disclosure on the silent mock backend, missing Stream of Thought / Sample response on the 12-second model load, weak Filters on the spectral-class slider, untranslated jargon as a Wayfinder failure). So the Metis frontend is *not* a clean slate for this audit — many of the gaps are already known and partly assigned. The new work is the systematic mapping, not the discovery.
- **Filed:** 2026-05-02 by claude/sharp-payne-90957a
- **Triage:**
  - **What it is:** Shape of AI is a *taxonomy + design-considerations* catalogue, not a component library and not a paper. It reads like a design-system handbook for AI-native products: each of the ~50 patterns is a normalised page with a one-paragraph definition, three to six design heuristics, links to related patterns, and a gallery of real-world example screenshots. The intent (per the About page) is to give designers "proven frameworks" so AI products don't reinvent interaction patterns ad hoc. The license is CC BY-NC-SA — reference and inspiration are fine, direct text/asset copy is not. The catalogue's strongest contributions for a project like Metis are the **Governors** family (Action plan, Stream of Thought, Branches, Cost estimates, Memory, Variations) which captures the human-in-the-loop discipline an always-on local companion *needs*, and the **Trust builders** family (Caveat, Disclosure, Footprints, Incognito Mode) which lines up tightly with Metis's "trace everything / local by default" principles. The **Tuners → Voice and tone / Personality** thread is the most interesting *missing piece* in Metis — the catalogue is explicit that voice/tone customisation is a first-class AI surface, and Metis currently has no UI for it (companion personality is implicit / data-driven, not user-tunable).
  - **Pillar fit:** **Cross-cutting** (🔧) primarily — this is a UI/UX yardstick, not a vision-pillar feature. Highest-leverage applications cluster on **Companion** (🌱 — Voice and tone, Personality, Memory controls, Disclosure on mock-backend / model-load, Caveat per-mode, Incognito Mode, Action plan for autonomous research) and **Cosmos** (🌌 — Wayfinders for empty constellation, Suggestions for first ingestion, Initial CTA on home, Filters that actually work). Some patterns don't fit Metis's vision and should be explicitly out-of-scope (Cost estimates is mostly about cloud-API token spend; Watermark is for generative-image products; Avatar/Personality face-customisation could fight the constellation-as-identity metaphor if mishandled).
  - **Overlap:** Broad. Probably touches every UI-bearing milestone:
    - **M01 (Rolling)** — natural home for the audit output. M01's *Web UI new-user audit (2026-04-25)* findings 24–31 (setup copy bloat, wizard voice inconsistency, jargon untranslated, "Seedling heartbeat" jargon) are *already* Shape-of-AI Wayfinder/Identifier/Disclosure gaps under a different label; the audit would re-frame them in catalogue terms and likely surface 5–10 net-new items.
    - **M21 (In progress)** — direct overlap. Several open items map cleanly onto Shape-of-AI patterns: P0 #5 (setup-key contradiction) = broken **Disclosure**; P1 #9 (12-second model-load with no progress feedback) = missing **Stream of Thought** + **Cost estimates** + **Sample response**; P2 #13 (browser-only setup state ignored) = missing **Memory**/**Modes** indicator. Audit findings should land as new P2/P3 entries inside M21 rather than a separate milestone.
    - **M13 (Landed)** — Seedling heartbeat widget is an in-flight **Stream of Thought** surface; the *Notes for the next agent* there should absorb the Shape-of-AI lens for the deferred-items audit (Memory controls, Caveat on always-on ingestion, Voice/tone for the companion).
    - **M14 (Landed)** — Forge already implements partial **Example gallery** + **Templates** + **Prompt details** patterns (technique cards, RUNTIME CHECK chips per M21 #22). Phase 7 (`.metis-skill` export) interacts with **Footprints** + **Saved styles** patterns.
    - **M16 (In progress)** — Personal evals naturally consume **Footprints** + **Variations** + **Sample response** patterns; the audit would shape its UI before it ships.
    - **M17 (Landed)** — Network audit panel is a textbook **Disclosure** + **Trust builders** surface; already aligned, may yield small copy refinements.
    - **M22 (Landed)** — Comet headline labels are a partial **Disclosure** + **Citations** pattern.
    - **No overlap** with backend milestones (M03–M11, M18 LoRA stretch).
  - **Risks worth noting:**
    - **License is CC BY-NC-SA.** No direct copy of pattern text into Metis docs/UI; the audit should reference patterns by name + URL only and write Metis-specific copy from scratch. Safe for prose summary in a plan doc; not safe for verbatim quotes in shipped UI.
    - **Catalogue completeness ≠ Metis fit.** ~50 patterns is a lot; not all apply to a local-first, lifestyle-business, no-image-generation, no-cloud-spend product. Cost estimates / Watermark / large parts of Inpainting/Restyle/Restructure are out of scope. The audit must explicitly mark N/A items so future agents don't chase them.
    - **Personality / Avatar are vision-tense.** VISION says "stars are knowledge, not astronomy" and the M-star mark is the *brand* identity — adding a chat-style avatar for the companion could dilute the constellation-as-identity metaphor. Voice/tone is safer than Avatar; tread carefully.
    - **Audit fatigue.** M01 + M21 already produced two long bug lists in the last week. A third audit pass risks producing more findings than capacity to fix. The right output is a *prioritised* short list (5–10 items), not a 50-row spreadsheet.
    - **Brainstorm-before-promote is the correct gate.** This idea is too big to plan as a single milestone without first seeing what the audit surfaces.
  - **Recommendation:** **Brainstorm before promote.** Specifically:
    1. Run a focused audit pass: walk each of the six pattern families against the existing Metis surface inventory (home `/`, `/chat`, `/setup`, `/forge`, `/settings`, `/library`, `/improvements`, observatory dialog, companion dock, network-audit panel). For each pattern, score ✅ shipped well · ⚠️ shipped but weak · ❌ missing · N/A out of scope. Half-day to 1 day if disciplined.
    2. Cross-reference findings against M01 audit notes + M21 bug inventory before promoting anything net-new — most weak/missing items already have a home.
    3. Output a single *Notes for the next agent* block in M01 with the full audit table + a "top 10 highest-leverage gaps" prioritised list. **Then** decide promote / merge for each top-10 item individually.
    4. **Most likely outcome:** ~6–8 of the top-10 items **Merge** into M21 as new P2/P3 entries; ~1–2 fold into M13/M14 *Notes for the next agent*; possibly **Promote** one genuine net-new item — **Companion voice/tone + memory controls panel** is the most likely candidate, because it's a *missing capability*, not a polish gap, and it ties directly to the Companion pillar.
    5. **Less likely:** if the audit surfaces a coherent multi-surface refactor (e.g. systematic Disclosure pass across mock-backend + model-load + setup-key + companion-stage transitions), **Promote** as a small **M23 — AI UX pattern coverage pass** milestone.
    6. Pin Shape of AI's URL + the audit table in M01's *Notes for the next agent* as a permanent reference even after the audit concludes — future UI work should sanity-check against the catalogue.
  - **Rough scope:**
    - **Audit + prioritisation:** half-day to 1 day.
    - **Patch path (most likely):** ~2–3 days spread across M01/M21/M13 plan-doc notes — many fixes are copy-only or one-component-deep.
    - **Net-new milestone path (less likely):** **multi-day** (~3–5) if Companion voice/tone + memory controls promotes; **multi-week** if a systematic Disclosure refactor lands.
    - **Permanent reference cost:** zero (URL pin in M01 notes).
- **Decision:** **Audit ran 2026-05-02; gaps #1 + #2 promoted as M23 on 2026-05-03; M23 implementation complete on 2026-05-03 (PR pending).** Full per-pattern scorecard + prioritised top-10 gap list at [`docs/preserve-and-productize-plan.md` → *Shape of AI pattern audit (2026-05-02)*](../docs/preserve-and-productize-plan.md). **Closed via M23:** gap #1 *Voice and tone + Personality* + gap #2 *Memory inspector* bundled as **M23 — Companion controls** ([`plans/companion-controls/plan.md`](companion-controls/plan.md), design at [`docs/plans/2026-05-03-companion-controls-design.md`](../docs/plans/2026-05-03-companion-controls-design.md), 25 commits, 6 phases, branch `claude/sharp-payne-90957a`). **Awaiting go/no-go (Merge into M21):** gap #3 *Caveat strip per mode*, gap #4 *Disclosure on AI-generated content*, gap #9 *Regenerate* (all P3-shaped polish). **Already in M21 under different names:** gap #5 *Filters that work* (= M21 #13) and gap #6 *Sample response / model-load progress* (= M21 #9) — re-tag with Shape-of-AI vocabulary when picked up. **Awaiting go/no-go (Park or Merge):** gap #7 *Action plan* (Park or Merge into M16), gap #8 *Incognito Mode* (Park or Merge into M17), gap #10 *Inline Action* (Park).
