...
- **Decision:** **Awaiting go/no-go.** Recommend brainstorm pass to scope this properly before any code lands. Specifically, my recommended next step is to run `superpowers:brainstorming` on sub-change (a) first — the faculty-removal scope question is foundational; everything else depends on what replaces faculty-as-placement.
