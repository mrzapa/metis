"""metis_app.cli — Headless command-line interface for METIS."""
...
def cmd_index(args: argparse.Namespace) -> int:
    ...
    out_path = pathlib.Path(args.out) if args.out else src.with_name(src.name + ".metis-index")
    try:
    ...
    try:
        if args.index:
            ...
            if str(bundle.vector_backend or "json") != "json":
                with tempfile.TemporaryDirectory(prefix="metis_cli_query_") as temp_dir:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="metis",
        description="METIS CLI — headless document indexing, querying, and skill management.",
        ...
        epilog=textwrap.dedent(
            ...
            examples:
              ...
              python -m metis_app.cli skills list
              python -m metis_app.cli skills enable research-claims
            """
        ),
    )
    ...
    p_index.add_argument(
        ...
        default=None,
        metavar="PATH",
        help="Output directory or manifest path for the persisted index (default: <file>.metis-index).",
    )
