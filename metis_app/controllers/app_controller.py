"""metis_app.controllers.app_controller — Top-level application controller.
...
via the poll loop in metis_app.app — never inside the worker thread.
"""
...
from metis_app.models.parity_types import SkillSessionState
...
from metis_app.services.local_llm_recommender import ImportPlan, LocalLlmRecommenderService
...
from metis_app.services.runtime_resolution import infer_file_types, resolve_runtime_settings
...
from metis_app.services.wizard_recommendation import recommend_auto_settings
from metis_app.utils.background import BackgroundRunner, CancelToken
from metis_app.utils.dependency_bootstrap import install_packages
from metis_app.utils.document_loader import KREUZBERG_EXTENSIONS, is_kreuzberg_available
from metis_app.utils.llm_providers import create_llm
from metis_app.utils.model_presets import list_llm_providers, provider_requires_custom_model

if TYPE_CHECKING:
    from metis_app.models.app_model import AppModel
    from metis_app.views.app_view import AppView
...
class AppController:
    ...
    def _ensure_test_mode_environment(self) -> str:
        if not self._test_mode_temp_dir:
            self._test_mode_temp_dir = tempfile.mkdtemp(prefix="metis_test_mode_")
    ...
    def _handle_message(self, msg: dict[str, Any]) -> None:
        ...
        if mtype == "status":
            ...
            if task == _TASK_BUILD_INDEX and isinstance(result, IndexBundle):
                ...
                header = (
                    f"METIS [{provider}, rag, mode={selected_mode}, "
                    f"{n_chunks} chunk(s)]:\n\n"
                )
    ...
    def on_send_prompt(self, prompt: str = "") -> None:
        ...
        def _rag_worker(post_msg: Any, cancel: CancelToken) -> dict[str, Any]:
            ...
            try:
                ...
                return {
                    "response": (
                        f"METIS [rag, mode={selected_mode}]: "
            ...
            if bool(settings_snap.get("enable_langextract")):
                grounding_html_path = build_grounding_html(
                    ...
                    title=f"METIS grounding · {selected_mode}",
                    query_text=prompt,
                    answer_text=answer,
                    sources=list(query_result.sources),
                )
    ...
    def _handle_direct_prompt(self, prompt: str) -> None:
        ...
        def _direct_worker(post_msg: Any, cancel: CancelToken) -> dict[str, str]:
            ...
            try:
                ...
                return {
                    "response": f"METIS [{provider_name}, direct]: {exc}\n\n",
                    "prompt": prompt,
                    "error": str(exc),
                }
            ...
            try:
                ...
                return {
                    "response": f"METIS [{provider_name}, direct]: LLM error: {exc}\n\n",
                    "prompt": prompt,
                    "error": str(exc),
                }
            self.trace_store.append_event(
