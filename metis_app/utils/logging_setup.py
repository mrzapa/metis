"""metis_app.utils.logging_setup — Application-wide logging initialisation.

Call ``setup_logging()`` once, as early as possible in ``metis_app.app``
(before Tk is created), so that even startup errors are captured on disk.

Hierarchy
---------
All metis_app modules use ``logging.getLogger(__name__)``, which produces
names like ``metis_app.models.app_model``.  These are children of the
``metis_app`` logger configured here, so they all inherit the two handlers
without any per-module configuration.

Handlers
--------
* **RotatingFileHandler** — writes DEBUG+ to ``<log_dir>/metis.log``.
  Rolls at 5 MB; keeps 3 backup files (``metis.log.1``, ``.2``, ``.3``).
...
# Root logger for the entire metis_app namespace.
_LOGGER_NAME = "metis_app"
...
def setup_logging(
    ...
    """Configure rotating-file + console logging for the metis_app namespace.

    Parameters
    ----------
    log_dir:
        Directory in which ``metis.log`` (and its rotated siblings) will be
    ...
    -------
    logging.Logger
        The configured ``metis_app`` root logger.  All child loggers
        (``metis_app.models.*``, ``metis_app.controllers.*``, …) inherit
