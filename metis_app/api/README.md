...
The API layer is a thin, typed boundary over METIS's engine layer.
...
```bash
python -m metis_app.api          # default: 0.0.0.0:8000
uvicorn metis_app.api:app --reload
```
...
HTTP 403 Forbidden
{
  "detail": "Updating API key fields is not permitted via this endpoint: ['api_key_openai']. Set METIS_ALLOW_API_KEY_WRITE=1 to override."
}
```
...
```bash
METIS_ALLOW_API_KEY_WRITE=1 python -m metis_app.api
```
...
```bash
METIS_API_CORS_ORIGINS="https://myapp.example.com,http://localhost:3000"
```
