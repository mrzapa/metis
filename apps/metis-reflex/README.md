...
- Python 3.10+
- The `metis_app` package available (either installed via the repo root or
...
```bash
# 1. From the repo root, install the metis_app core (skip if already installed)
pip install -e .

# 2. Enter the prototype directory
cd apps/metis-reflex
...
# export METIS_LLM_MODEL=gpt-4o-mini
...
|----------|---------|-------------|
| `METIS_LLM_PROVIDER` | `anthropic` | LLM provider (`anthropic`, `openai`, …) |
| `METIS_LLM_MODEL` | `claude-haiku-4-5-20251001` | Model name |
