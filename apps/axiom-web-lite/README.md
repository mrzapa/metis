...
full design parity with `apps/axiom-web`. The goal is a fast proof-of-concept that
...
- The Python API running locally:
  ...
  ```
- At least one index built via `apps/axiom-web` or the Python CLI (the chat page
...
`PUBLIC_AXIOM_API_BASE` to override:

```bash
PUBLIC_AXIOM_API_BASE=http://localhost:9000 npm run dev
```
...
- No sharing of code or state with `apps/axiom-web`.
...
workspace. Building or running it has no effect on `apps/axiom-web`.
