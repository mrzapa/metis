"""Axiom Reflex Prototype — experimental web UI over the existing core.

Provides session management, direct and RAG queries, a document library with
index building, and a settings panel — all wired to the axiom_app engine.
"""
...
class State(rx.State):
    ...
    def load_indexes(self):
        from axiom_app.engine.index_registry import list_indexes
    ...
    def run_rag_query(self):
        from axiom_app.engine.querying import RagQueryRequest, query_rag
    ...
    def load_settings(self):
        from axiom_app.settings_store import load_settings
    ...
    def save_settings(self):
        from axiom_app.settings_store import save_settings
    ...
    def build_index(self):
        from axiom_app.engine.indexing import IndexBuildRequest, build_index
        ...
        settings = {
            "llm_provider": os.environ.get("AXIOM_LLM_PROVIDER", "anthropic"),
            "llm_model": os.environ.get("AXIOM_LLM_MODEL", "claude-haiku-4-5-20251001"),
            "embedding_provider": os.environ.get("AXIOM_EMBEDDING_PROVIDER", ""),
            "api_key_anthropic": os.environ.get("ANTHROPIC_API_KEY", ""),
            "api_key_openai": os.environ.get("OPENAI_API_KEY", ""),
        }
        try:
...
def index() -> rx.Component:
    return rx.vstack(
        nav_bar(),
        rx.vstack(
            rx.heading("Axiom Reflex", size="7", margin_bottom="0.5em"),
            rx.text(
                "Session management, direct queries, and RAG queries against the Axiom engine.",
                color_scheme="gray",
                size="2",
                margin_bottom="1em",
            ),
...
def settings_page() -> rx.Component:
    return rx.vstack(
        ...
        rx.vstack(
            ...
            rx.text(
                "View and update Axiom engine settings.",
                color_scheme="gray",
                size="2",
                margin_bottom="1em",
            ),
