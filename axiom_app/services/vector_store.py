...
def weaviate_test_settings_from_env(env: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    settings = {
        "weaviate_url": source.get("AXIOM_TEST_WEAVIATE_URL", ""),
        "weaviate_api_key": source.get("AXIOM_TEST_WEAVIATE_API_KEY", ""),
        "weaviate_grpc_host": source.get("AXIOM_TEST_WEAVIATE_GRPC_HOST", ""),
        "weaviate_grpc_port": source.get("AXIOM_TEST_WEAVIATE_GRPC_PORT", ""),
        "weaviate_grpc_secure": source.get("AXIOM_TEST_WEAVIATE_GRPC_SECURE", ""),
    }
        ...
        *,
        ...
        if target_path is not None:
            ...
            if path.suffix.lower() == ".json" and path.name != "manifest.json":
                from axiom_app.services.index_service import save_index_bundle
