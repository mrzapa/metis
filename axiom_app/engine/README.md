...
- No behavior is implemented in `axiom_app.engine` yet.
...
| --- | --- |
| `IndexBundle` | `axiom_app.services.index_service.IndexBundle` |
| `IndexManifest` | `axiom_app.models.parity_types.IndexManifest` |
| `QueryResult` | `axiom_app.services.index_service.QueryResult` |
| `EvidenceSource` | `axiom_app.models.session_types.EvidenceSource` |
| `SessionSummary` | `axiom_app.models.session_types.SessionSummary` |
| `SessionMessage` | `axiom_app.models.session_types.SessionMessage` |
| `SessionFeedback` | `axiom_app.models.session_types.SessionFeedback` |
| `SessionDetail` | `axiom_app.models.session_types.SessionDetail` |
| `TraceEvent` | `axiom_app.models.parity_types.TraceEvent` |
...
- The current `axiom_app.services.trace_store.TraceStore` reads dict payloads; the engine wrapper can normalize them into `TraceEvent` instances.
...
| --- | --- | --- |
| `build_index(...)` | `axiom_app/services/vector_store.py` and `axiom_app/services/index_service.py::build_index_bundle` | Current adapters already route build requests to the shared index builder. |
| `save_index(...)` | `axiom_app/services/vector_store.py` and `axiom_app/services/index_service.py::persist_index_bundle` / `save_index_bundle` | Engine should return `IndexManifest` even if a backend currently returns a manifest path. |
| `refresh_index(...)` | `axiom_app/services/index_service.py::refresh_index_bundle` | Existing function rewrites persisted bundle and manifest metadata. |
| `list_indexes(...)` | `axiom_app/services/index_service.py::list_index_manifests` and `axiom_app/services/vector_store.py::VectorStoreAdapter.list_indexes` | Engine should standardize on `list[IndexManifest]`. |
| `load_index(...)` | `axiom_app/services/index_service.py::load_index_bundle` and `axiom_app/services/vector_store.py::VectorStoreAdapter.load` | Supports manifest-backed directories and legacy JSON bundles today. |
| `query_index(...)` | `axiom_app/services/index_service.py::query_index_bundle` and backend adapter `query(...)` methods in `axiom_app/services/vector_store.py` | Existing retrieval already returns typed `QueryResult`. |
| `stream_query(...)` retrieval phase | `axiom_app/services/index_service.py::query_index_bundle` or backend adapter query routing in `axiom_app/services/vector_store.py` | Engine should emit a typed retrieval event before synthesis. |
| `stream_query(...)` synthesis phase | `axiom_app/services/response_pipeline.py` and `axiom_app/utils/llm_providers.py::create_llm` | Current controller uses `run_blinkist_summary_pipeline`, `run_tutor_pipeline`, and `llm.invoke(...)`. |
| `stream_query(...)` trace emission | `axiom_app/services/trace_store.py` plus `axiom_app.models.parity_types.TraceEvent` | Current controller appends trace events around retrieval, synthesis, validation, and grounding. |
| `SessionStore` | `axiom_app/services/session_repository.py::SessionRepository` and repo-root `rag_sessions.db` | Current persistence already returns `SessionSummary` and `SessionDetail`. |
| `TraceStore` | `axiom_app/services/trace_store.py::TraceStore`, repo-root `traces/runs.jsonl`, and `traces/runs/<run_id>.jsonl` | Current trace persistence is append-only JSONL. |
| Session trace hydration | `SessionRepository.get_session(...)` plus `TraceStore.read_runs(...)`, currently composed in `axiom_app/controllers/app_controller.py::_session_trace_payload` | This composition should move out of the UI controller and behind the engine boundary. |
