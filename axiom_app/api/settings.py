...
def post_settings(payload: SettingsUpdateRequest) -> dict[str, Any]:
    ...
    environment variable ``AXIOM_ALLOW_API_KEY_WRITE=1`` is explicitly set.
    ...
    if denied and os.getenv("AXIOM_ALLOW_API_KEY_WRITE", "").strip() != "1":
        raise HTTPException(
            status_code=403,
            detail=(
                f"Updating API key fields is not permitted via this endpoint: {denied}. "
                "Set AXIOM_ALLOW_API_KEY_WRITE=1 to override."
            ),
        )
    try:
