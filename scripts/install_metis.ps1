...
<#
.SYNOPSIS
    METIS Installer for Windows — install, reinstall, or uninstall the METIS app.

.DESCRIPTION
    Downloads (or updates) the METIS repository, creates a Python virtual
...
.EXAMPLE
    .\install_metis.ps1
    .\install_metis.ps1 -Action reinstall
    .\install_metis.ps1 -Action uninstall
...
$PipRetries  = if ($env:METIS_PIP_RETRIES) { [int]$env:METIS_PIP_RETRIES } else { 10 }
$PipTimeout  = if ($env:METIS_PIP_TIMEOUT) { [int]$env:METIS_PIP_TIMEOUT } else { 120 }
$InstallRetries = if ($env:METIS_INSTALL_RETRIES) { [int]$env:METIS_INSTALL_RETRIES } else { 4 }
$LauncherDir = Join-Path $HOME ".local" "bin"
$LauncherPs1 = Join-Path $LauncherDir "metis.ps1"
$LauncherCmd = Join-Path $LauncherDir "metis.cmd"

# ── Helpers ──────────────────────────────────────────────────────────────────
function Write-Info  { param([string]$Msg) Write-Host "[metis] $Msg" -ForegroundColor Cyan }
function Write-Ok    { param([string]$Msg) Write-Host "[metis] $Msg" -ForegroundColor Green }
function Write-Warn  { param([string]$Msg) Write-Host "[metis] $Msg" -ForegroundColor Yellow }
function Write-Err   { param([string]$Msg) Write-Host "[metis] $Msg" -ForegroundColor Red }
...
#   metis --cli <args>     -- CLI mode (args forwarded)
`$ErrorActionPreference = "Stop"
`$metisDir    = "$InstallDir"
`$branch      = "$Branch"
...
`$webDir      = Join-Path `$metisDir "apps\metis-web\out"

function Show-Help {
    Write-Host "METIS launcher"
    Write-Host ""
    Write-Host "Usage:"
    Write-Host "  metis                  Start the local API and static web UI"
    Write-Host "  metis --desktop        Start the legacy Qt desktop shell"
    Write-Host "  metis --gui            Alias for --desktop"
    Write-Host "  metis --cli <args>     Run the CLI"
    Write-Host "  metis --help           Show this help"
}
...
Push-Location `$metisDir
try {
    if (`$desktopMode) {
        & `$venvPython (Join-Path `$metisDir "main.py") @launchArgs
        exit `$LASTEXITCODE
    }

    if (`$cliMode) {
        & `$venvPython (Join-Path `$metisDir "main.py") "--cli" @launchArgs
        exit `$LASTEXITCODE
    }
}
finally {
...
if (-not (Test-Path (Join-Path `$webDir "index.html"))) {
    throw [System.InvalidOperationException]::new("Built web UI not found at `$webDir. Install Node.js and re-run the installer, or build apps/metis-web manually.")
}
...
if ((Test-PortInUse -HostName `$apiHost -Port `$apiPort) -or (Test-PortInUse -HostName `$webHost -Port `$webPort)) {
    if ((Test-UrlReady -Url `$apiHealthUrl) -and (Test-UrlReady -Url `$webUrl)) {
        ...
        Write-Host "METIS is already running at `$webUrl."
        exit 0
    }
...
try {
    ...
    Start-Process `$webUrl
    Write-Host "METIS running (API PID `$(`$apiProcess.Id), Web PID `$(`$webProcess.Id)) at `$webUrl. Press Ctrl+C to stop."
...
"@ | Set-Content -Path $LauncherPs1 -Encoding UTF8
    ...
    # CMD wrapper so `metis` works from cmd.exe too.
    @"
@echo off
REM Auto-generated METIS launcher - do not edit.
...
"%PS_BIN%" -NoProfile -ExecutionPolicy Bypass -File "%~dp0metis.ps1" %*
exit /b %ERRORLEVEL%
...
function Invoke-Uninstall {
    ...
    Write-Ok "METIS has been uninstalled."
}
...
function Invoke-Install {
    ...
    # ── Web UI (optional) ────────────────────────────────────────────────
    $WebAppDir = Join-Path $InstallDir "apps" "metis-web"
    ...
    # ── Summary ──────────────────────────────────────────────────────────
    Write-Host ""
    Write-Host "METIS installed successfully!" -ForegroundColor Green -BackgroundColor Black
    Write-Host ""
    Write-Info "Install directory : $InstallDir"
    Write-Info "Virtual env       : $VenvDir"
    Write-Host ""
    Write-Info "Run METIS:"
    Write-Host "  metis                            # Launch METIS"
    Write-Host "  metis --cli index --file f.txt   # CLI mode"
    Write-Host ""
...
function Invoke-Update {
    ...
    if (-not (Test-Path $gitDir)) {
        Write-Err "METIS is not installed at $InstallDir. Run install first."
        exit 1
    }
    ...
    Write-Ok "METIS updated to latest."
}

# ── Main ─────────────────────────────────────────────────────────────────────
Write-Host ""
Write-Host "=== METIS Installer ===" -ForegroundColor Cyan
Write-Host ""
